...
const ReviewerSignin = ({ setIsSignupOpen, setIsSigninOpen }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await axios.post("/reviewer/login", {
                email,
                password,
                role: "reviewer",
    ...
    return (
        <>
            <section id="signin-reviewer-modal" className="fixed top-0 left-0 w-full h-full flex items-center justify-center backdrop-blur-lg bg-[rgba(17,22,28,0.8)] z-50">
                <div className="w-[40vw] flex flex-col items-center justify-center bg-white shadow-lg rounded-xl p-8">
                    ...
                    <form onSubmit={handleSubmit} className="flex flex-col gap-4 w-full openSans">
                        ...
                        <button type="submit" className="bg-gradient-to-r from-blue-500 to-purple-600 text-white px-4 py-2 rounded-lg cursor-pointer transition-all duration-300 hover:scale-105">Signin as Reviewer</button>
                    </form>
                </div>
            </section>
        </>
