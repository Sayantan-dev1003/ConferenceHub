...
import DeleteSpeakerAcc from './DeleteSpeakerAcc';
...
const SettingsSpeaker = () => {
  ...
  const handleSaveProfile = async () => {
    try {
      await axios.put('/api/update/speaker', speakerData);
  ...
  const handleChangePassword = async () => {
    try {
      ...
      const response = await axios.post('/api/passwordchange/speaker', { oldPassword, newPassword }, { withCredentials: true });
  ...
  return (
    <div className='w-full min-h-screen flex justify-end items-start openSans'>
            ...
            <div className='bg-white p-6 rounded-lg shadow-md'>
              <div className='mb-4 flex justify-between gap-4'>
                <div className='w-1/2'>
                  ...
                  <input type='text' name='fullname' value={speakerData.fullname} onChange={handleProfileChange} className='border border-gray-300 rounded-md w-full p-2' />
                </div>
                <div className='w-1/2'>
                  <label className='block text-gray-700'>Email</label>
                  <input type='email' name='email' value={speakerData.email} onChange={handleProfileChange} className='border border-gray-300 rounded-md w-full p-2' />
                </div>
              </div>
              <div className='mb-4 flex justify-between gap-4'>
                <div className='w-1/2'>
              ...
              <div className='mb-4 flex justify-between gap-4'>
                ...
                <div className='w-1/2'>
                  ...
                  <input type='text' name='location' value={speakerData.location} onChange={handleProfileChange} className='border border-gray-300 rounded-md w-full p-2' />
                </div>
              </div>
              <div className='mb-4'>
                <label className='block text-gray-700'>Social Media Links</label>
                <div className='flex justify-between gap-4'>
                  <input type='text' name='twitter' placeholder='Twitter' value={speakerData.socialMediaLinks.twitter} onChange={(e) => handleProfileChange({ target: { name: 'socialMediaLinks', value: { ...speakerData.socialMediaLinks, twitter: e.target.value } } })} className='border border-gray-300 rounded-md w-full p-2' />
                  <input type='text' name='linkedin' placeholder='LinkedIn' value={speakerData.socialMediaLinks.linkedin} onChange={(e) => handleProfileChange({ target: { name: 'socialMediaLinks', value: { ...speakerData.socialMediaLinks, linkedin: e.target.value } } })} className='border border-gray-300 rounded-md w-full p-2' />
                </div>
              </div>
      ...
      <DeleteSpeakerAcc isOpen={isDeleteModalOpen} onClose={() => setDeleteModalOpen(false)} />
    </div>
  )
}
